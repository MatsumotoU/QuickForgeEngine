...
void QFE::AnimationPlayer::Update(float deltaTime){
	deleteAnimations_.clear();

	// 現在再生中のアニメーションを更新
	activeAnimations_.Each([this, deltaTime](uint32_t id, AnimationPlayClip& playClip) {
		if (playClip.isPlaying) {
			...
			if (playClip.isLoop) {
				...
				deleteAnimations_.push_back(id);
...
void QFE::AnimationPlayer::FrameEnd(){
	// アニメーションを破棄する
	for(uint32_t& id : deleteAnimations_) {
