...
uint32_t ModelVertexResourceManager::AssignPlane(ID3D12Device* device, float width, float height, uint32_t segmentsX, uint32_t segmentsY, bool invertFace) {
	// 同じ名前のモデルが既に存在する場合はそのハンドルを返す
	...
	// メッシュデータを作成する
	MeshData planeMesh = CreatePlane(width, height, segmentsX, segmentsY, invertFace);
	// 頂点データを割り当てる
	modelVertexBuffers_.emplace_back();
...
uint32_t ModelVertexResourceManager::AssignBox(ID3D12Device* device, bool invertFace)
{
	// 同じ名前のモデルが既に存在する場合はそのハンドルを返す
    ...
    // メッシュデータを作成する
	MeshData boxMesh = CreateBox(invertFace);
...
uint32_t ModelVertexResourceManager::AssignRing(ID3D12Device* device, float innerRadius, float outerRadius, uint32_t segments, bool invertFace) {
    // 同じ名前のモデルが既に存在する場合はそのハンドルを返す
    ...
    // メッシュデータを作成する
    MeshData ringMesh = CreateRing(innerRadius, outerRadius, segments, invertFace);
    // 頂点データを割り当てる
    modelVertexBuffers_.emplace_back();
