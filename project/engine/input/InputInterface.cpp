...
namespace QFE::INPUT {
	...
	/** @brief キーによる移動方向を取得 */
	MATH::Vector2 InputInterface::GetKeyMoveDir() {
	...
	/** @brief マウスの移動量を取得 */
	MATH::Vector2 InputInterface::GetMouseMove() {
		return directInputManager_.mouse_.mouseMoveDir_;
	}

	/** @brief マウスのスクリーン座標を取得 */
	MATH::Vector2 InputInterface::GetMouseScreenPos() {
	...
	MATH::Vector2 InputInterface::GetGamePadLeftStickDir() {
		return xInputController_.GetLeftStick(0);
	}

	/** @brief ゲームパッドの右スティック方向を取得 */
	MATH::Vector2 InputInterface::GetGamePadRightStickDir() {
