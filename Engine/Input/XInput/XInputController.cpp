...
Vector2 XInputController::GetLeftStick(uint32_t padId) {
	...
	if (stickDeadZone_ >= MAX_STICK) {
		stickDeadZone_ = MAX_STICK;
	}
	...
	Vector2 result{};
	float rawX = static_cast<float>(gamepadStates[padId].state.Gamepad.sThumbLX);
	float rawY = static_cast<float>(gamepadStates[padId].state.Gamepad.sThumbLY);

	// デッドゾーン処理
	if (std::fabsf(rawX) < stickDeadZone_) rawX = 0.0f;
	else rawX = (rawX - std::copysign(stickDeadZone_, rawX)) / (MAX_STICK - stickDeadZone_);

	if (std::fabsf(rawY) < stickDeadZone_) rawY = 0.0f;
	else rawY = (rawY - std::copysign(stickDeadZone_, rawY)) / (MAX_STICK - stickDeadZone_);

	// 範囲クランプ
	rawX = std::clamp(rawX, -1.0f, 1.0f);
	rawY = std::clamp(rawY, -1.0f, 1.0f);
